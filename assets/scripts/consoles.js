const APICo = "http://localhost:3001/consoles";
...
btnBuscar.onclick = () => {
    ...
    const filtrados = consoles.filter((u) => u.nome && u.nome.toLowerCase().includes(termo));
...
async function carregarconsoles() {
    try {
        ...
        const resposta = await fetch(APICo);
        const dados = await resposta.json();

        if (Array.isArray(dados)) {
            consoles = dados;
        } else if (Array.isArray(dados.consoles)) {
            consoles = dados.consoles;
        } else {
            consoles = [];
        }

        mostrarconsoles(consoles);

    } catch (error) {
      console.error("Erro ao carregar consoles:", error);
      if (gamesList) gamesList.innerHTML = "<p>Erro ao carregar consoles.</p>";
    }
  }
...
function mostrarconsoles(lista) {
  if (!gamesList) return;
  gamesList.innerHTML = "";

  lista.forEach(jg => {
    const card = document.createElement("section");
    card.className = "todosconsoles";
    ...
    if (btnVoltar) {
        btnVoltar.addEventListener('click', () => {
            if (window.__originalMainContent) {
                ...
                // re-executa carregarconsoles para re-attach listeners, se necessário
                carregarconsoles();
            }
        });
    }
